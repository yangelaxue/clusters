"""
Copied and pasted from: https://astro.ucla.edu/~wright/CC.python
Modified for compatibility for the current Python version.
...
try:
  if sys.argv[1] == '-h':
    ...
    print('input values = redshift, Ho, Omega_m, Omega_vac')
    print('ouput values = age at z, distance in Mpc, kpc/arcsec, apparent to abs mag conversion')
    print('Options:   -h for this message    -v for verbose response')
    sys.exit()
  if sys.argv[1] == '-v':
    verbose=1
...
# comoving volume computation
  ...
  if verbose == 1:
    print('For H_o = ' + '%1.1f' % H0 + ', Omega_M = ' + '%1.2f' % WM + ', Omega_vac = %1.2f' % WV + ', z = ' + '%1.3f' % z)
    ...
    print('The comoving radial distance, which goes into Hubbles law, is %1.1f' % DCMR_Mpc + ' Mpc or ' + '%1.1f' % DCMR_Gyr + ' Gly.')
    print('The comoving volume within redshift z is ' + '%1.1f' % V_Gpc + ' Gpc^3.')
    print('The angular size distance D_A is ' + '%1.1f' % DA_Mpc + ' Mpc or %1.1f' % DA_Gyr + ' Gly.')
