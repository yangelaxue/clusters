...
class file_spectrum:
  def __init__(self,specfname='kol_spectrum.txt'):
    filespec = np.loadtxt(specfname)
    kfile=filespec[:,0]*100
    efile=filespec[:,1]*1e-6
...
class pq_spectrum:
  ...
  def evaluate(self, k):
    ...
    return espec
# class kolmogorov_spectrum:
#   def __init__(self):
#     kolspec = np.loadtxt('kol_spectrum.txt')
#     kkol=kolspec[:,0]*100
#     ekol=kolspec[:,1]*1e-6
#     self.especf = interpolate.interp1d(kkol, ekol,'cubic')
#     self.kmin = kkol[0]
#     self.kmax = kkol[len(kkol) - 1]
#   def evaluate(self,k):
#     return self.especf(k)